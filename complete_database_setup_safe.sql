-- =====================================================
-- EchoStyle Complete Database Setup for Supabase (SAFE VERSION)
-- =====================================================
...
DROP POLICY IF EXISTS "Users can view their own skin analyses" ON skin_care_analyses;
DROP POLICY IF EXISTS "Users can insert their own skin analyses" ON skin_care_analyses;
DROP POLICY IF EXISTS "Users can update their own skin analyses" ON skin_care_analyses;
DROP POLICY IF EXISTS "Users can delete their own skin analyses" ON skin_care_analyses;
...
DROP POLICY IF EXISTS "Anyone can view seasonal palettes" ON seasonal_palettes;
...
BEGIN
    ...
    -- Enable RLS on skin_care_analyses table
    ...
    -- Enable RLS on seasonal_palettes table
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'products') THEN
        CREATE POLICY IF NOT EXISTS "Anyone can view products" ON products
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'skin_care_analyses') THEN
        CREATE POLICY IF NOT EXISTS "Users can view their own skin analyses" ON skin_care_analyses
          FOR SELECT USING (auth.uid() = user_id);
        
        CREATE POLICY IF NOT EXISTS "Users can insert their own skin analyses" ON skin_care_analyses
          FOR INSERT WITH CHECK (auth.uid() = user_id);
        
        CREATE POLICY IF NOT EXISTS "Users can update their own skin analyses" ON skin_care_analyses
          FOR UPDATE USING (auth.uid() = user_id);
        
        CREATE POLICY IF NOT EXISTS "Users can delete their own skin analyses" ON skin_care_analyses
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'analysis_history') THEN
        CREATE POLICY IF NOT EXISTS "Users can view their own analysis history" ON analysis_history
          FOR SELECT USING (auth.uid() = user_id);
        
        CREATE POLICY IF NOT EXISTS "Users can insert their own analysis history" ON analysis_history
          FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'saved_products') THEN
        CREATE POLICY IF NOT EXISTS "Users can view their own saved products" ON saved_products
          FOR SELECT USING (auth.uid() = user_id);
        
        CREATE POLICY IF NOT EXISTS "Users can insert their own saved products" ON saved_products
          FOR INSERT WITH CHECK (auth.uid() = user_id);
        
        CREATE POLICY IF NOT EXISTS "Users can delete their own saved products" ON saved_products
          FOR DELETE USING (auth.uid() = user_id);
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_profiles') THEN
        CREATE POLICY IF NOT EXISTS "Users can view their own profiles" ON user_profiles
          FOR SELECT USING (auth.uid() = user_id);
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'seasonal_palettes') THEN
        CREATE POLICY IF NOT EXISTS "Anyone can view seasonal palettes" ON seasonal_palettes
          FOR SELECT USING (true);
    END IF;
END $$;
